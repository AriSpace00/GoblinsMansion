...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    float3 EmissiveLighting = txEmissive.Sample(samplerLinear, input.Texcoord).rgb;
    ...
    DirectionLighting += (DiffuseBRDF + SpecularBRDF) * Color * NDotL;
    ...
    if (uv.x >= 0.0 && uv.x <= 1.0 && uv.y >= 0.0 && uv.y <= 1.0)
    {
        ...
        if (currentShadowDepth < 1 && currentShadowDepth > sampleShadowDepth + 0.001)
        {
    ...
    float3 final = DirectionLighting + AmbientLighting + PointLighting + EmissiveLighting; //    +EmissiveLighting; //saturate(directionLighting + BaseColor);
