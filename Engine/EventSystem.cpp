...
void EventSystem::Tick(World* world, ECS::DefaultTickData data)
{
	...
	if (info && player)
	{
		if (CheckRoom(info, 2))
		{
			if (IsColliding(info, "room_02_triggercoll_01") && IsLooking(info,"movementText_01_01") && m_CurrentEventIndex != 1)
			{
		...
		if (CheckRoom(info, 3))
		{
			if (IsColliding(info, "room_03_triggercoll_01") && (IsLooking(info, "Door_01") || IsLooking(info, "Door_02")) && m_CurrentEventIndex != 2)
			{
				m_CurrentEventIndex = 2;
				world->emit<Events::SpaceReturn>({ 2 });
				world->emit<Events::SpaceReturn>({ 100 });
				world->emit<Events::SpaceAssemble>({ 3,4,1,0 });
				world->emit<Events::SpaceAssemble>({ 4,2,1,0 });
				world->emit<Events::SpaceAssemble>({ 5,100,1,0 });
			}
		...
		if(CheckRoom(info, 100))
		{
			if (IsColliding(info, "room_100_triggercoll_01") && IsLooking(info, "movementText_01_02") && m_CurrentEventIndex != 50)
			{
				m_CurrentEventIndex = 50;
				world->emit<Events::SpaceReturn>({ 1 });
				world->emit<Events::SpaceReturn>({ 4 });
				world->emit<Events::SpaceReturn>({ 5 });
				world->emit<Events::SpaceAssemble>({ 100,3,0,1 });
				world->emit<Events::SpaceAssemble>({ 3,6,0,0 }); 
			}
		}
		...
		if (CheckRoom(info, 6))
		{
			if (IsColliding(info, "room_06_triggercoll") && (IsLooking(info, "Door_01") || IsLooking(info, "Door_02")) && info->m_Room6Check && m_CurrentEventIndex != 4)
			{
				m_CurrentEventIndex = 4;
				info->m_Room6Check = false;
				world->emit<Events::SpaceReturn>({ 2 });
		...
		if(CheckRoom(info, 7))
		{
			if(IsColliding(info, "room_07_triggercoll"))
			{
				if(IsLooking(info, "room_07_wall_08") && m_CurrentEventIndex != 5)
				{
					m_CurrentEventIndex = 5;
					world->emit<Events::SpaceAssemble>({ 7,8,1,0 });				
				}
				if(m_CurrentEventIndex != 6)
				{
			...
			if((IsLooking(info, "chest_obj_01") || IsLooking(info, "chest_obj_02") || IsLooking(info, "chest_obj_03")) && info->m_Room7Check >= 3)
			{
