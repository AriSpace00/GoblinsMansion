...
{
	RenderManager::GetInstance()->AddDynamicText(event.entity->getEntityId(), event.component->m_Text[event.component->m_TextNum], event.component->m_FontIndex);
}

void DebugSystem::Receive(World* world, const Events::DynamicTextChange& event)
{
	auto component = event.entity->get<DynamicText>();
	RenderManager::GetInstance()->EditDynamicText(event.entity->getEntityId(), component->m_CurrentTextIndex, component->m_FontIndex, component->m_IsTextShow);
}
