...
struct cbPointLight
{
	...
	void Clear()
	{
		...
		{
			pointLights[i].mRadius = 0.f;
			pointLights[i].mIntensity = 0.f;
		}
	}
};
...
public:
	...
	//spritefont 렌더용
	std::unique_ptr<DirectX::SpriteFont> m_shillaFont;
	std::unique_ptr<DirectX::SpriteFont> m_ulsanFont;
...
public:
	...
	void AddDynamicTextInformation(int entId, const vector<std::wstring>& vector, int fontIndex);
	...
	void EditDynamicTextInformation(int id, int textIndex, int fontIndex, bool enable);
...
void CreateShadowPS(); private:
	...
	const wchar_t* m_shillaPath = L"../Resource/font/shilla_b.spritefont";
	const wchar_t* m_ulsanjungguPath = L"../Resource/font/ulsanjunggu.spritefont";

	vector<TextInformation> m_texts;
	vector<DynamicTextInformation> m_dynamicTexts;
	XMVECTORF32 m_textColor = DirectX::Colors::White;

	void ChangeFont(int fontIndex, Vector2 fontPos, const wchar_t* text, int currentTextIndex, int currentTextSize);
};