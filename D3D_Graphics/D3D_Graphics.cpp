...
void Renderer::AddDynamicTextInformation(int entId, const vector<std::wstring>& vector, int fontIndex)
{
	m_dynamicTexts.push_back({ entId, 0, fontIndex, false, vector });
}
...
void Renderer::EditDynamicTextInformation(int id, int textIndex, int fontIndex, bool enable)
{
	...
	it->mTextIndex = textIndex;
	it->mFontIndex = fontIndex;
	it->mEnable = enable;
}
...
void Renderer::ChangeFont(int fontIndex, Vector2 fontPos, const wchar_t* text, int currentTextIndex, int currentTextSize)
{
	wchar_t currentText[6];
	swprintf(currentText, 6, L"%d / %d", currentTextIndex, currentTextSize);
	float size = 6 * (20.f * 1.5f) / 2;
	XMFLOAT2 currentTextPos = { (float)m_fontPosX - size, (float)m_fontPosY + 100.f };

	wchar_t spaceText[17];
	swprintf(spaceText, 17, L"Space 키 눌러서 넘어가기");
	float size2 = 17 * (20.f * 1.5f) / 2;
	XMFLOAT2 spaceTextPos = { (float)m_fontPosX - size2, (float)m_fontPosY + 200.f };

	if (fontIndex == 0)
	{
		m_textColor = DirectX::Colors::White;
		m_ulsanFont->DrawString(m_spriteBatch.get(), currentText, currentTextPos, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 1.5f);
		m_ulsanFont->DrawString(m_spriteBatch.get(), text, fontPos, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 1.5f);
		m_ulsanFont->DrawString(m_spriteBatch.get(), spaceText, spaceTextPos, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 1.5f);
		return;
	}
	m_textColor = DirectX::Colors::DarkRed;
	m_shillaFont->DrawString(m_spriteBatch.get(), currentText, currentTextPos, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 1.5f);
	m_shillaFont->DrawString(m_spriteBatch.get(), text, fontPos, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 1.5f);
	m_shillaFont->DrawString(m_spriteBatch.get(), spaceText, spaceTextPos, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 1.5f);
}
...
void Renderer::RenderText() const
{
	for (int i = 0; i < m_texts.size(); i++)
	{
		...
		m_shillaFont->DrawString(m_spriteBatch.get(), text, { (float)m_fontPosX , (float)m_fontPosY }, m_textColor, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 0.5f);
	...
	for (int i = 0; i < m_dynamicTexts.size(); i++)
	{
		...
		{
			const wchar_t* text = m_dynamicTexts[i].mText[m_dynamicTexts[i].mTextIndex].c_str();
			int length = m_dynamicTexts[i].mText[m_dynamicTexts[i].mTextIndex].size();
			float size = length * (20.f * 1.5f) / 2;
			XMFLOAT2 pos = { (float)m_fontPosX - size, (float)m_fontPosY };
			int currentTextSize = m_dynamicTexts[i].mText.size();
	...
	m_shillaFont->DrawString(m_spriteBatch.get(), systemMemory, DirectX::XMFLOAT2(0.f, 20.f), DirectX::Colors::White, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 0.7f);
	delete[] systemMemory;
	...
	m_shillaFont->DrawString(m_spriteBatch.get(), wFPS, DirectX::XMFLOAT2(0.f, 40.f), DirectX::Colors::Black, 0.f, DirectX::XMFLOAT2(0.f, 0.f), 0.7f);
	delete[] wFPS;
...
bool Renderer::Initialize(HWND* hWnd, UINT width, UINT height)
{
	...
	m_shillaFont = std::make_unique<DirectX::SpriteFont>(m_pDevice.Get()
		, m_shillaPath);
	m_ulsanFont = std::make_unique<DirectX::SpriteFont>(m_pDevice.Get()
		, m_ulsanjungguPath);
	...
	pBackBufferMaterial->GetDesc(&testDesc);
	m_fontPosX = testDesc.Width / 2;
	m_fontPosY = testDesc.Height / 2;
