		...
		for (auto it = entity.begin(); it != entity.end(); ++it)
		{
			...
			for (const auto& component : it.value())
			{
				...
				else if (componentName == "Light")
				{
					...
					loadEntity->Assign<Light>(type);
					loadEntity->get<Light>().get().m_ComponentName = component["Light"][0]["m_ComponentName"];
					loadEntity->get<Light>().get().m_Color = component["Light"][0]["m_Color"];
					loadEntity->get<Light>().get().m_Direction = component["Light"][0]["m_Direction"];
					loadEntity->get<Light>().get().m_Intensity = component["Light"][0]["m_Intensity"];
					loadEntity->get<Light>().get().m_Radius = component["Light"][0]["m_Radius"];
				}
				else if (componentName == "Movement")
				{
...
void GameEditor::NewScene()
{
	...
	player->Assign<PlayerInformation>();
	...
	ent->get<Movement>()->m_Speed = 600.f;	// 100은 답답하니 좀 올려놓자.
...
void GameEditor::PlayScene()
{
	...
	m_ActiveWorld->registerSystem(new UISystem);
