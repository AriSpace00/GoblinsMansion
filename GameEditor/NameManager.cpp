...
void NameManager::DeleteEntityName(ECS::Entity* entity)
{
	for (auto iter = m_NameContainer.begin(); iter != m_NameContainer.end(); iter++)
	{
		if (*iter == entity->get<EntityIdentifier>()->m_EntityName)
		{
			m_NameContainer.erase(iter);
			break;
		}
	}
